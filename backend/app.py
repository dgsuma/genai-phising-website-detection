...
class GenAIResult(BaseModel):
    genai_score: float = Field(..., ge=0.0, le=100.0, description="Phishing risk score from 0 (safe) to 100 (definite phishing)")
...
class ScanResponse(BaseModel):
    ...
    genai_summary: GenAIResult  # ← Now accepts the structured object directly
...
IMPORTANT RULES:
- Do NOT mark a URL suspicious just because metadata (title/snippet/brand/context) is missing.
- Missing information does NOT imply phishing.
- If the domain is well-known and uses HTTPS, default to SAFE unless strong phishing indicators exist.
- Be conservative: only mark PHISHING when clear and strong signals are present.
- genai_score should be a number between 0 (completely safe) and 100 (definite phishing).

Return structured output only — no extra text.
"""
...
def scan(req: ScanRequest):
    ...
    # 7) Optional GenAI override (only if not trusted domain)
    if not trusted:
